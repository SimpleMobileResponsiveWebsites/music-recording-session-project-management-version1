...
selected_tool = st.sidebar.selectbox("Select Instrument/Tool", INSTRUMENTS_TOOLS)
session_filter = st.sidebar.selectbox("Session Status Filter", ["All", "Planned", "In Progress", "Completed"])
st.sidebar.write("---")
...
if "sessions" not in st.session_state or reset_data:
    st.session_state.sessions = pd.DataFrame(columns=["Session Name", "Musicians", "Equipment", "Status", "Date", "Notes"])
...
if session_filter == "All":
    ...
    filtered_sessions = st.session_state.sessions[
        st.session_state.sessions["Status"].str.contains(session_filter, case=False, na=False)
    ]
